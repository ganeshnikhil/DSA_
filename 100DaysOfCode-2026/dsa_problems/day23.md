<h2 align="center">Day 23 (08/07/2025)</h2>
...
A problem that introduces **stream processing**, **stateful pattern detection**, and **real-time bit manipulation**. It teaches how to **prevent reserved patterns** from appearing in a bit stream by inserting **stuffing bits dynamically**.

Given a **binary string** representing a data stream and a **pattern** that should never appear in the transmitted data, your task is to **insert a `0` bit immediately after the pattern occurs** in the stream. If the pattern never occurs, the stream remains unchanged.

This problem appears frequently in **networking applications**, such as **HDLC frame transmission**, **CAN bus communication**, and other **protocol implementations** that require data transparency and error-free communication.

You can solve this by checking for the pattern using **substring matching**, but a more **efficient approach** uses **progressive pattern matching** or **stateful scanning**, processing **one bit at a time** and inserting the stuffing bit **on-the-fly**.

This teaches **stream processing**, **stateful algorithms**, and **pattern prevention techniques** that are essential for **communication protocol implementations**.

**Your task:** Implement bit stuffing for a given binary stream and a forbidden pattern. The stuffed stream should **never contain the pattern** in its original form.
